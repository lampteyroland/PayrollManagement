        ...
        <form method="POST" action="/salaries/{{$salary->id}}" class="mb-5" enctype="multipart/form-data">
            ...
            <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                <div class="sm:col-span-3">
                    <label for="employee_id" class="block text-sm font-medium leading-6 text-gray-900">Pay Frequency</label>
                    <div class="mt-2">
                        <select name="employee_id" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6">
                            ...
                            @foreach($employees as $employee)
                                <option value="{{ $employee->id }}" {{ $salary->employee_id == $employee->id ? 'selected' : '' }}>{{ $employee->first_name }}</option>
                            @endforeach
                        </select>
                        @error('employee_id')
                ...
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-2">
                        <select name="pay_frequency" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6">
                            <option value="">Select Frequency</option>
                            <option value="Annually" {{ $salary->pay_frequency == 'Annually' ? 'selected' : '' }}>Annually</option>
                            ...
                            <option value="Bi-weekly" {{ $salary->pay_frequency == 'Bi-weekly' ? 'selected' : '' }}>Bi-weekly</option>
                            <option value="Quarterly" {{ $salary->pay_frequency == 'Quarterly' ? 'selected' : '' }}>Quarterly</option>
                        </select>
                    </div>
                    @error('pay_frequency')
                    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
