<x-app-layout>
    ...
    <x-card class=" pr-10 pl-6 w-full">
        ...
        <div >
            <form method="POST" action="/taxes/{{$tax->id}}" class="mb-5 ">
                @csrf
                @method('PUT')

                <h2 class="mt-10 text-base font-semibold leading-7 text-gray-900">Employee Tax Information Information</h2>
                <p class="text-sm leading-6 text-gray-600">Edit tax information of employee</p>
                    ...
                    <div class="mt-2">
                        <select name="employee_id" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6">
                            ...
                            @foreach($employees as $employee)
                                <option value="{{ $employee->id }}" @if(old('employee_id', $tax->employee_id) == $employee->id) selected @endif>{{ $employee->first_name . ' '. $employee->last_name}}</option>
                            @endforeach
                        </select>
                        @error('employee_id')
                ...
                <div class="mt-3 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    <div class="sm:col-span-2">
                        ...
                        <div class="mt-2">
                            <input type="text" name="tax_rate" value="{{ $tax->tax_rate }}"
                                   class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                        </div>
                        @error('tax_rate')
                        <p class="text-red-500 text-xs mt-1">{{$message}}</p>
              ...
              <div class="sm:col-span-3">
                    ...
                    <div class="mt-2">
                        {{dd($tax)}}
                        <select   name="tax_type"  {{ $tax->tax_type }} class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6">
                            <option value="">Select Tax Type</option>
                            <option value="Type 1" {{  $tax->tax_type == 'Type 1' ? 'selected' : ''}}>Type 1</option>
                            <option value="Type 2" {{   $tax->tax_type == 'Type 2' ? 'selected' : '' }}>Type 2</option>
                            <option value="Type 3" {{   $tax->tax_type == 'Type 3' ? 'selected' : '' }}>Type 3</option>
                            <option value="Type 4" {{  $tax->tax_type == 'Type 4' ? 'selected' : '' }}>Type 4</option>
                            <option value="Type 5" {{  $tax->tax_type == 'Type 5' ? 'selected' : '' }}>Type 5</option>
                            <option value="Type 6" {{  $tax->tax_type == 'Type 6' ? 'selected' : '' }}>Type 6</option>
                        </select>
                    </div>
                    @error('tax_type')
                    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                ...
                <div class="mt-3 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    <div class="sm:col-span-3">
                        <label for="tax_date" class="block text-sm font-medium leading-6 text-gray-900">Tax Date</label>
                        <div class="mt-2">

                            <input type="date" name="tax_date" value="{{$tax->tax_date}}"
                                   class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                        </div>
                        @error('tax_date')
                        <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                ...
                <div class="mt-3 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    <div class="sm:col-span-3">
                        ...
                        <div class="mt-2">
                            <input type="text" name="description" value="{{ $tax->description }}"
                                   class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                        </div>
                        @error('description')
                        <p class="text-red-500 text-xs mt-1">{{$message}}</p>
